...
## Intel Virtualization Technology,Enabled;[Optional:Disabled,Enabled]
$Name = "Intel Virtualization Technology"
$output = (Get-LenovoBIOSSettings | Select-String $name )

$State = "Intel Virtualization Technology,Enabled;[Optional:Disabled,Enabled]"
if ((Get-WmiObject win32_computersystem).Manufacturer -notmatch "LENOVO") {Return "Compliant"}
elseif(Compare-Object $State $output -ExcludeDifferent -IncludeEqual){Return "Compliant"}
