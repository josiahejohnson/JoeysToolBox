...
Param(
    ...
    [parameter(ParameterSetName='Extra',mandatory=$true)]
        [alias("NetworkPath")]
            [string]$netPath,
    [parameter(ParameterSetName='Extra',Mandatory=$true)]
        [alias("creds")][System.Management.Automation.PSCredential][System.Management.Automation.Credential()]
...
$foldername = "SystemInfoPage-" + (get-date -Format ddMMMyyyy).ToString()
...
If($networkSave)
    {
        ...
        If(!(Test-Path -Path $filepath -Credential $Credential)){new-item -Path "$netPath" -Name "$foldername" -Credential $Credential -ItemType Directory -Force | out-null }
    }
Else
    {
        $filepath = "$env:USERPROFILE" + "\documents"
        If(!(Test-Path -Path "$filepath\$folderName")){new-item -Path "$filepath" -Name "$FolderName" -ItemType Directory -Force | out-null }
    }
